...
// Plugin defines the standard for all plugins
type Plugin interface{}

// ResolverPlugin defines the standard for all server discovery plugins
type ResolverPlugin interface {
	Init(...Option) error
}

// TracingPlugin defines the standard for all tracing plugins
type TracingPlugin interface {
	Init(...Option) (opentracing.Tracer, error)
}

// PluginMap defines a global plugin map
var PluginMap = make(map[string]Plugin)

// Register opens an entry point for all plugins to register

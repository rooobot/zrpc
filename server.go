...
import (
	...
	"github.com/rooobot/zrpc/plugin"
	"github.com/rooobot/zrpc/plugin/jaeger"
	"os"
	"os/signal"
	"reflect"
	"syscall"
...
// zrpc Server, a Server can have one or more Services
type Server struct {
	opts    *ServerOptions
	service Service
...
func NewServer(opt ...ServerOption) *Server {
	...
	for pluginName, plg := range plugin.PluginMap {
...
func checkMethod(method reflect.Type) error {
	...
	if argType.Kind() != reflect.Ptr {
		return fmt.Errorf("method %s invalid, req typq is not a pointer", method.Name())
	}
